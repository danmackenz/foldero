...
4. **Execute one at a time.** On Approve or Redirect: `mv` to destination, verify (source gone AND destination present), log per `references/Undo-Rules.md` §2 with action `signoff-approve`. On Reject: remove queue entry, log with action `signoff-reject`. Never overwrite — collisions per `references/Collision-Handling.md`.
5. **Update documents.** Remove the resolved entry from INDEX's `Awaiting sign-off` section. Append to INDEX's `Activity` section and `_LOGS/activity-log.md` (dated, per-item, with the user's decision).
6. **Never open contents.** Tier 3 items are classified by name/metadata/structure only. Sign-off does not require or invite content inspection.
